...
class FTPServer(object):
    ...
    def run_forever(self):
        ...
        while True:
            ...
            run_logger.info('got a new connection from %s' % (self.addr,))
    ...
    def handle(self):
        ...
        while True:
            ...
            if not raw_data:
                ...
                run_logger.error('connection %s is lost....' % (self.addr, ))
                error_logger.error('connection %s is lost....' % (self.addr, ))
    ...
    def send_response(self, status_code, *args, **kwargs):
        ...
        if len(bytes_data) < self.MSG_SIZE:
            ...
            bytes_data = json.dumps(data).encode('utf-8')
        run_logger.info("pack data send to %s" % (self.addr,))
    ...
    def _mkdir(self, data):
        ...
        cmd_result = stdout + stderr
        self.send_response(351, cmd_result_size=len(cmd_result))
        self.request.sendall(cmd_result)
    ...
    def _rm(self, data):
        ...
        cmd_result = stdout + stderr
        self.send_response(352, cmd_result_size=len(cmd_result))
        self.request.sendall(cmd_result)
    ...
    def _cd(self, data):
        ...
        if os.path.isdir(full_path):
            ...
            if full_path.startswith(self.user_obj['home']):
                ...
                self.send_response(351)    # 无权限直接提示目录不存在
